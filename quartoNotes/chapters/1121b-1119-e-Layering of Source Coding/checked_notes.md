...
$$
当且仅当 $M = 2^n$（如4, 8, 16）时，$\log_2 M$ 为整数，此时 $L = \log_2 M$。
...
```mermaid
graph TD
    A[所有编码] --> B[非奇异编码<br>(Non-singular)]
    A --> C[奇异编码]
    B --> D[唯一可译编码<br>(Uniquely Decodable)]
    D --> E[即时编码/前缀码<br>(Instantaneous/Prefix-free)]
    E --> F[最优前缀码<br>(Optimal Prefix-free)]
```
...
### 数学形式（PPT图17）
对于任意一组码字长度 $\{l(x)\}_{x \in \mathcal{X}}$，若存在一个前缀码使其成立，则必满足：
$$
